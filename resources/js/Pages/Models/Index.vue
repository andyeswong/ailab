        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg"
                    v-if="reactive_user.models != undefined">
                    <div class="p-6 text-gray-900 dark:text-gray-100" v-for="model in reactive_user.models">
                        <div class="flex">
                            <div class="flex-grow">
                                ...
                                <p class="text-gray-600 dark:text-gray-400">
                                    ...
                                    <span v-if="modelMetricsLastObject(model.model_metrics).train_loss != undefined"
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 bg-green-100 text-green-800 mr-1">
                                        Val Loss: {{ modelMetricsLastObject(model.model_metrics).train_loss }}
                                    </span>

                                    <!-- tailwind pill  -->
                                    <span v-if="model.model_hyperparameters != undefined"
                            ...
                            <div class="flex-shrink-0">
                                <a v-if="model.status == 'trained'" :href="`/models/${model.id}`"
                                    class="inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-blue-500 hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue active:bg-blue-700 transition ease-in-out duration-150">
                                    Explore
                                </a>
                                <!-- button disabled with status yellow -->
                                <button v-else
                                    class="inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-yellow-500 hover:bg-yellow-700 dark:bg-yellow-600 dark:hover:bg-yellow-700 focus:outline-none focus:border-yellow-700 focus:shadow-outline-yellow active:bg-yellow-700 transition ease-in-out duration-150"
                                    @click="openConsole(model)">
                                    {{ model.status }}
                                </button>
                                <button
                                    class=" ml-1 inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-green-500 hover:bg-green-700 dark:bg-green-600 dark:hover:bg-green-700 focus:outline-none focus:border-green-700 focus:shadow-outline-green active:bg-green-700 transition ease-in-out duration-150"
                                    @click="openConsole(model)">
                                    Training console
                                </button>
                            </div>
